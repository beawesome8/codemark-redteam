...
def score_result(raw_text: str, expected: dict) -> dict:
    ...
    try:
        validated = CRMSummary(**json.loads(clean_output(raw_text)))
        return {
            "schema_valid": 1,
...
def run_prompt_against_set(
    ...
    for i, case in enumerate(cases):
        ...
        if not quiet:
            ...
            sentiment = "OK" if score["sentiment_correct"] else "MISS"
            urgency = "OK" if score["urgency_correct"] else "MISS"
            print(
                f"  [{i + 1:02d}/{len(cases):02d}] {case['id']} — "
                f"schema:{status} sentiment:{sentiment} urgency:{urgency} "
...
def main():
    ...
    print_report(
        compute_summary(baseline_results),
        compute_summary(candidate_results),
