...
from storage.init_db import DB_PATH, init_db
...
def build_system_prompt(prompt_config: dict) -> str:
    """Constructs the full system prompt, including any few-shot examples."""
...
def call_claude(prompt_config: dict, user_input: str) -> dict:
    ...
    system = build_system_prompt(prompt_config)
...
def score_result(raw_text: str, expected: dict) -> dict:
    ...
    try:
        parsed = json.loads(clean_output(raw_text))
...
def save_result(
    ...
    conn.execute(
        ...
        (
            ...
            case_id,
            score["schema_valid"],
            score["sentiment_correct"],
            score["urgency_correct"],
            api_result["latency_ms"],
            api_result["cost_usd"],
            api_result["input_tokens"],
            api_result["output_tokens"],
            api_result["raw_text"],
            score["parsed_sentiment"],
            score["parsed_urgency"],
            api_result["error"] or score.get("parse_error"),
        ),
    )


def run_prompt_against_set(
    prompt_config: dict,
    cases: list,
    run_id: str,
    run_type: str,
    ...
    for i, case in enumerate(cases):
        ...
        if not quiet:
            schema_status = "PASS" if score["schema_valid"] else "FAIL"
            sentiment_status = "OK" if score["sentiment_correct"] else "MISS"
            urgency_status = "OK" if score["urgency_correct"] else "MISS"
            print(
...
def compute_summary(results: list) -> dict:
    ...
    return {
        ...
        "schema_valid_pct": round(
            sum(r["schema_valid"] for r in results) / total * 100, 1
        ),
        "sentiment_acc_pct": round(
            sum(r["sentiment_correct"] for r in results) / total * 100, 1
        ),
        "urgency_acc_pct": round(
            sum(r["urgency_correct"] for r in results) / total * 100, 1
        ),
    }
...
def print_report(
    ...
    for label, key, unit in metrics:
        ...
        candidate_val = candidate_summary[key]
        delta = round(candidate_val - baseline_val, 1)
        ...
        print(
            f"  {label:<25} {baseline_val:>9}{unit} "
            f"{candidate_val:>9}{unit} {sign}{delta:>8}{unit}{flag}"
        )
    ...
    if "BLOCK" in decisions:
        ...
        decision = "WARNING – review before merging"
    else:
        decision = "PASSED – safe to merge"
...
def main():
    ...
    parser.add_argument(
        "--ci-mode", action="store_true", help="Suppress per-case output for CI logs"
    )
